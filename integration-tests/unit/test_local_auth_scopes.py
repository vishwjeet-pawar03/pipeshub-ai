"""The integration test client must be granted every scope the backend defines.
...
SCOPES_ENUM = (
    ...
    / "backend/nodejs/apps/src/libs/enums/oauth-scopes.enum.ts"
)


def _backend_scopes() -> set[str]:
    source = SCOPES_ENUM.read_text()
    block = source[source.index("OAuthScopeNames") :]
...
def test_the_test_client_is_granted_every_backend_scope() -> None:
    ...
    assert backend, f"No scopes parsed from {SCOPES_ENUM}"
