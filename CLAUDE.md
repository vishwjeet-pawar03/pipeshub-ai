...
- **Indexing** (`backend/python`, port 8091) — `app.indexing_main`. Parse, chunk, embed; write records through `IGraphDBProvider` and `IVectorDBService` (not a hardcoded Qdrant or Arango client). Consumer loop, heavy/light tiers, admission control, ResourceGovernor, and the throughput-collapse root cause are documented in `docs/indexing-service.md`; read it before reviewing changes under `app/services/messaging/`, `app/services/resource_governor/`, `app/events/`, or `app/modules/transformers/`.
