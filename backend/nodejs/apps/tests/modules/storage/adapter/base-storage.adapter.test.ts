  ...
  describe('deleteTree', () => {
    it('should delegate to the underlying adapter', async () => {
      ...
      const adapterWithDelete = new StorageServiceAdapter({
        ...mockStorageService,
        deleteTree: deleteStub,
      })

      const result = await adapterWithDelete.deleteTree('records/conn-1/file')

      expect(result.statusCode).to.equal(200)
      expect(deleteStub.calledOnceWith('records/conn-1/file')).to.be.true
    })

    it('should reject with error message when deleteTree is not implemented', async () => {
      ...
      try {
        await adapterNoDelete.deleteTree('records/conn-1/file')
        expect.fail('Should have rejected')
      } catch (error: any) {
        expect(error.message).to.equal('deleteTree not implemented for this storage provider')
      }
    })
  })
