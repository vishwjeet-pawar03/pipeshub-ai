...
class TestFetchGroups:
    ...
    async def test_fetches_group_members(self):
        ...
        account_ids, ok = await connector._fetch_group_members("g1", "developers")
        assert ok is True
...
class TestExtractIssueData:
    ...
    def test_epic_detection(self):
        ...
        # is_epic/is_subtask flags were removed from the return dict; epic-ness now lives
        # in the mapped issue_type, and an epic has no parent.
    ...
    def test_subtask_detection(self):
        ...
        result = connector._extract_issue_data(issue, {})
        # is_subtask flag removed; a sub-task is identified by its extracted parent id.
...
class TestHandleIssueDeletions:
    ...
    async def test_with_sync_time(self):
        ...
        connector.issues_sync_point.read_sync_point = AsyncMock(return_value=None)
        connector.issues_sync_point.update_sync_point = AsyncMock()
        # _detect_and_handle_deletions now returns (checkpoint_ms, success); the checkpoint
...
class TestFetchAttachmentAsBase64:
    ...
    @pytest.mark.asyncio
    async def test_image_inlined(self):
        connector = _make_connector()
        mock_ds = MagicMock()
    ...
    async def test_oversized_returns_none(self):
        ...
        cache = {}
        result = await connector._fetch_attachment_as_base64(
            {"id": "9", "mimeType": "image/png", "size": 999_999_999}, cache
        )
        assert result is None
    ...
    async def test_cache_hit_skips_fetch(self):
        connector = _make_connector()
        connector._get_fresh_datasource = AsyncMock(side_effect=AssertionError("should not fetch"))
...
class TestFetchDeletedIssuesFromAudit:
    ...
    async def test_audit_api_failure(self):
        ...
        # Non-OK audit page -> ok is False so the caller won't advance past unread deletions.
        keys, ok = await connector._fetch_deleted_issues_from_audit("2024-01-01", "2024-01-02")
        assert keys == []
        assert ok is False
...
class TestDetectAndHandleDeletions:
    ...
    async def test_no_deletions(self):
        connector = _make_connector()
        # audit fetch now returns (keys, ok); detect returns (checkpoint_ms, success).
    ...
    async def test_deletion_error_continues(self):
        ...
        # window; both issues are still attempted.
        _checkpoint_ms, success = await connector._detect_and_handle_deletions(1700000000000)
