...
class TestGetAccessibleVirtualRecordIds:
    ...
    async def test_merges_results_across_tasks(self, provider):
        ...
        async def fake_connector(user, org, cid, metadata, time_range=None, raise_on_error=False):
            return {"v1": "r1", "v2": "r2"}

        async def fake_kb(user, org, kb_ids, metadata, time_range=None, raise_on_error=False):
    ...
    async def test_task_exceptions_are_tolerated(self, provider):
        async def failing(user, org, cid, metadata, time_range=None, raise_on_error=False):
            raise RuntimeError("boom")

        async def ok(user, org, kb_ids, metadata, time_range=None, raise_on_error=False):
            return {"v1": "r1"}
