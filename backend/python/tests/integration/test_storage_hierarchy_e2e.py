...
SERVICENOW_CONNECTOR_ID = "conn-servicenow-001"
...
class StorageGraphProvider:
    ...
    def __init__(self) -> None:
        ...
        self._parent_edges: list[tuple[str, str]] = []  # (child_id, parent_id) for records
        self._group_parent_edges: list[tuple[str, str]] = []  # (child_group_id, parent_group_id) BELONGS_TO
    ...
    def add_record_group_parent(self, child_group_id: str, parent_group_id: str) -> "StorageGraphProvider":
        self._group_parent_edges.append((child_group_id, parent_group_id))
        return self
    ...
    async def get_record_group_path(
        ...
        group = self._record_groups.get(record_group_id)
        if not group:
            return []
        names: list[str] = []
        ...
        if name:
            names.append(name)
        current = record_group_id
        visited = {current}
        while True:
            parent_id = None
            for child, parent in self._group_parent_edges:
                if child == current:
                    parent_id = parent
                    break
            ...
            parent_group = self._record_groups.get(parent_id)
            if not parent_group:
                break
            pname = parent_group.get("groupName") or parent_group.get("name", "")
            if pname:
                names.append(pname)
            current = parent_id
        names.reverse()
...
# ---------------------------------------------------------------------------
# Tests: Nested record group hierarchy (ServiceNow-style)
# ---------------------------------------------------------------------------


class TestNestedRecordGroupHierarchy:
    """Exercises nested BELONGS_TO edges between record groups — the pattern
    connectors like ServiceNow, Zammad, and Bookstack use."""

    @pytest.mark.asyncio
    async def test_servicenow_style_kb_category_subcategory(self):
        """KB('IT') → Category('Android') → Subcategory('Eclipse') → record."""
        provider = StorageGraphProvider()
        provider.add_record_group("grp-it", "IT")
        provider.add_record_group("grp-android", "Android")
        provider.add_record_group("grp-eclipse", "Eclipse")
        provider.add_record_group_parent("grp-android", "grp-it")
        provider.add_record_group_parent("grp-eclipse", "grp-android")
        provider.add_record("rec-article", "Eclipse configuration")

        record = SimpleNamespace(
            connector_id=SERVICENOW_CONNECTOR_ID,
            record_group_id="grp-eclipse",
            id="rec-article",
            record_name="Eclipse configuration",
            virtual_record_id=None,
            connector_name=None,
            weburl=None,
        )
        path = await build_hierarchical_storage_path(record, provider)
        assert path == f"records/{SERVICENOW_CONNECTOR_ID}/IT/Android/Eclipse/Eclipse configuration"

    @pytest.mark.asyncio
    async def test_deep_nesting_four_levels(self):
        """Root → Level1 → Level2 → Level3 → record."""
        provider = StorageGraphProvider()
        provider.add_record_group("grp-root", "Root")
        provider.add_record_group("grp-l1", "Level1")
        provider.add_record_group("grp-l2", "Level2")
        provider.add_record_group("grp-l3", "Level3")
        provider.add_record_group_parent("grp-l1", "grp-root")
        provider.add_record_group_parent("grp-l2", "grp-l1")
        provider.add_record_group_parent("grp-l3", "grp-l2")
        provider.add_record("rec-deep", "deep-file.txt")

        record = SimpleNamespace(
            connector_id=SERVICENOW_CONNECTOR_ID,
            record_group_id="grp-l3",
            id="rec-deep",
            record_name="deep-file.txt",
            virtual_record_id=None,
            connector_name=None,
            weburl=None,
        )
        path = await build_hierarchical_storage_path(record, provider)
        assert path == f"records/{SERVICENOW_CONNECTOR_ID}/Root/Level1/Level2/Level3/deep-file.txt"

    @pytest.mark.asyncio
    async def test_rename_nested_group_updates_path(self):
        """Renaming a leaf group updates the full ancestor chain."""
        provider = StorageGraphProvider()
        provider.add_record_group("grp-parent", "Parent")
        provider.add_record_group("grp-child", "OldName")
        provider.add_record_group_parent("grp-child", "grp-parent")
        provider.add_record("rec-1", "file.txt")

        record = SimpleNamespace(
            connector_id=SERVICENOW_CONNECTOR_ID,
            record_group_id="grp-child",
            id="rec-1",
        ...
        old_path = await build_hierarchical_storage_path(record, provider)
        assert old_path == f"records/{SERVICENOW_CONNECTOR_ID}/Parent/OldName/file.txt"

        provider.rename_record_group("grp-child", "NewName")
        new_path = await build_hierarchical_storage_path(record, provider)
        assert new_path == f"records/{SERVICENOW_CONNECTOR_ID}/Parent/NewName/file.txt"

    @pytest.mark.asyncio
    async def test_placeholder_replaced_with_real_name(self):
        """Placeholder group (name = external ID) replaced with real name."""
        provider = StorageGraphProvider()
        provider.add_record_group("grp-placeholder", "ext-id-12345")
        provider.add_record_group("grp-child", "Articles")
        provider.add_record_group_parent("grp-child", "grp-placeholder")
        provider.add_record("rec-1", "doc.pdf")
        ...
        path_before = await build_hierarchical_storage_path(record, provider)
        assert path_before == f"records/{SERVICENOW_CONNECTOR_ID}/ext-id-12345/Articles/doc.pdf"

        provider.rename_record_group("grp-placeholder", "Real Category Name")
        path_after = await build_hierarchical_storage_path(record, provider)
        assert path_after == f"records/{SERVICENOW_CONNECTOR_ID}/Real Category Name/Articles/doc.pdf"

    @pytest.mark.asyncio
    async def test_single_group_no_nesting(self):
        """A record group with no parent edges still works."""
        provider = StorageGraphProvider()
        provider.add_record_group("grp-solo", "Solo Group")
        provider.add_record("rec-1", "file.txt")

        record = SimpleNamespace(
            connector_id=SERVICENOW_CONNECTOR_ID,
            record_group_id="grp-solo",
            id="rec-1",
            record_name="file.txt",
            virtual_record_id=None,
            connector_name=None,
        ...
        path = await build_hierarchical_storage_path(record, provider)
        assert path == f"records/{SERVICENOW_CONNECTOR_ID}/Solo Group/file.txt"

    @pytest.mark.asyncio
    async def test_orphaned_parent_edge_falls_back(self):
        """Parent group doesn't exist → only the child name is used."""
        provider = StorageGraphProvider()
        provider.add_record_group("grp-child", "ChildOnly")
        provider.add_record_group_parent("grp-child", "nonexistent-parent")
        provider.add_record("rec-1", "file.txt")
        ...
        path = await build_hierarchical_storage_path(record, provider)
        assert path == f"records/{SERVICENOW_CONNECTOR_ID}/ChildOnly/file.txt"

    @pytest.mark.asyncio
    async def test_get_record_group_path_traverses_full_chain(self):
        """Directly verify the in-memory provider's traversal."""
        provider = StorageGraphProvider()
        provider.add_record_group("grp-a", "Alpha")
        provider.add_record_group("grp-b", "Beta")
        provider.add_record_group("grp-c", "Gamma")
        provider.add_record_group_parent("grp-b", "grp-a")
        provider.add_record_group_parent("grp-c", "grp-b")

        result = await provider.get_record_group_path("grp-c")
        assert result == ["Alpha", "Beta", "Gamma"]

    @pytest.mark.asyncio
    async def test_get_record_group_path_single_group(self):
        """No parent edges → single-element list."""
        provider = StorageGraphProvider()
        provider.add_record_group("grp-solo", "Solo")
        result = await provider.get_record_group_path("grp-solo")
        assert result == ["Solo"]

    @pytest.mark.asyncio
    async def test_get_record_group_path_nonexistent(self):
        """Nonexistent group → empty list."""
        provider = StorageGraphProvider()
        result = await provider.get_record_group_path("does-not-exist")
        assert result == []

    def test_build_record_group_prefix_from_chain(self):
        """build_record_group_prefix_from_chain composes the full prefix."""
        assert (
            build_record_group_prefix_from_chain("conn-1", ["Root", "Child", "Leaf"])
            == "records/conn-1/Root/Child/Leaf"
        )

    def test_build_record_group_prefix_from_chain_empty(self):
        assert build_record_group_prefix_from_chain("conn-1", []) is None

    def test_build_record_group_prefix_from_chain_sanitizes(self):
        result = build_record_group_prefix_from_chain("conn-1", ["A:B", "C\nD"])
        assert result == "records/conn-1/A_B/C_D"

    @pytest.mark.asyncio
    async def test_rename_parent_group_affects_all_descendants(self):
        """Renaming a mid-level group changes paths for records under children."""
        provider = StorageGraphProvider()
        provider.add_record_group("grp-root", "KB")
        provider.add_record_group("grp-mid", "OldCategory")
        provider.add_record_group("grp-leaf", "SubCat")
        provider.add_record_group_parent("grp-mid", "grp-root")
        provider.add_record_group_parent("grp-leaf", "grp-mid")
        provider.add_record("rec-1", "article.html")

        record = SimpleNamespace(
            connector_id=SERVICENOW_CONNECTOR_ID,
            record_group_id="grp-leaf",
            id="rec-1",
            record_name="article.html",
            virtual_record_id=None,
            connector_name=None,
            weburl=None,
        )
        old_path = await build_hierarchical_storage_path(record, provider)
        assert old_path == f"records/{SERVICENOW_CONNECTOR_ID}/KB/OldCategory/SubCat/article.html"

        provider.rename_record_group("grp-mid", "NewCategory")
        new_path = await build_hierarchical_storage_path(record, provider)
        assert new_path == f"records/{SERVICENOW_CONNECTOR_ID}/KB/NewCategory/SubCat/article.html"
...
class TestMergeTimeRangeOptimization:
    ...
    async def test_time_range_filters_before_blob_fetch(self):
        ...
        result_vrids = [r["virtual_record_id"] for r in results]
        assert "vrid-new" in result_vrids
        assert "vrid-old" not in result_vrids
    ...
    async def test_no_time_range_fetches_all(self):
        ...
        result_vrids = {r["virtual_record_id"] for r in results}
        assert result_vrids == {"vrid-1", "vrid-2"}
