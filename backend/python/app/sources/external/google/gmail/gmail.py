...
class GoogleGmailDataSource:
    ...
    async def execute(self, operation: Callable[[], T]) -> T:
        ...
        if self._execute_lock.locked():
            logger.debug(
                "Gmail datasource %#x: execute lock busy, request queuing behind "
                "an in-flight call",
                id(self),
            )
        async with self._execute_lock:
            logger.debug("Gmail datasource %#x: execute lock acquired", id(self))
            ...
            try:
                ...
                logger.debug("Gmail datasource %#x: execute lock releasing", id(self))
