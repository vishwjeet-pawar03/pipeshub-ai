...
@mcp_server(
    MCPServerTemplate(
        type_id="gmail",
        display_name="Gmail",
        description="Search threads, read email, create drafts, and manage labels in a Gmail account.",
        ...
        # The toolset drafts and labels rather than sends, so `gmail.send` authorizes none
        # of it. Google documents `gmail.compose`, which covers drafts but not the
        # label/unlabel tools; `gmail.modify` is the narrowest scope spanning both.
        default_scopes=[
        ...
        auth_hint=AuthHint(
            label="Google OAuth",
            help_text=(
                "Enable both the Gmail API (gmail.googleapis.com) and the Gmail MCP API "
                "(gmailmcp.googleapis.com) on your Google Cloud project, then create an OAuth "
