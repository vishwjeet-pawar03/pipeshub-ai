...
def _answer_markdown_description() -> str:
    ...
    return (
        ...
        "Retrieved/fetched content is wrapped in `<record>...</record>`, one per document, "
        "with metadata (Record ID, Name, Type, Web URL) at the top. Blocks inside are compact: "
        "`[idx|refN] content` (search results — a jump in `idx` means blocks in between were "
        "not retrieved, not that they don't exist) or `[refN] content` (full record reads, "
        "already contiguous). `[Table #g: summary]` / `[<Type> #g]` mark a group header "
        "followed by its child blocks. `refN` is always the citable id. Web results use "
        "`url/Citation ID: https://ref271.xyz` instead — same citation rules, own token shape.\n\n"
        "**Citation rules**\n"
        ...
        "- Keep prose readable: cite the specific claim it backs, not every clause or sentence "
        "in a row — a paragraph built from one source needs one citation at its end, not one "
        "per sentence. Never stack multiple `[source](...)` links back to back; if several "
        "blocks support the same claim, pick the single best one."
    )
