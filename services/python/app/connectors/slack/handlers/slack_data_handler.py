"""
Optimized Slack Data Handler with Proper Kafka Event Implementation
- Fixed workspace-organization edge creation
- Proper Kafka events for messages and files
- Maintains user message grouping and all existing functionality
- Files sent as separate events to Kafka
- Correct event structure matching change handler pattern
"""
...
class SlackDataHandler:
    ...
    async def _sync_workspace_info(self, workspace_id: str, org_id: str, transaction = None) -> Optional[str]:
        ...
        try:
            ...
            if existing_workspace:
                ...
                # FIXED: Create edge between workspace and organization
                workspace_org_edge = {
                    "_from": f"{CollectionNames.SLACK_WORKSPACES.value}/{workspace_key}",
                    "_to": f"orgs/{org_id}",  # Fixed: using correct org collection reference
    ...
    async def get_sync_health(self, workspace_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if health_status["sync_lag_minutes"] > 60:
